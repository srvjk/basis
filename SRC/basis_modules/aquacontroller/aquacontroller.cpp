...
public:
	...
	std::mutex sensorDataMutex; /// мьютекс дл€ защиты операций с данными сенсоров
	asio::streambuf readBuf;    /// буфер дл€ чтени€ из последовательного порта
	std::atomic_bool readerBusy = false;
	std::unique_ptr<std::thread> serialPortThread; /// поток дл€ работы с последовательным портом
...
struct WindowData
{
	int width = 1024; /// текуща€ ширина окна приложени€
	int height = 786; /// текуща€ высота окна приложени€
};
